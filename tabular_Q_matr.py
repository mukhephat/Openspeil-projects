...
#________________
  ...
  def step(self, time_step, is_evaluation=False):
    ...
    if self._prev_info_state and not is_evaluation:
      ...
      yy = self.unconvertBoard(y)
      print('Unconverted board\n',yy)        
      maxPrev = 0
      for item in self._q_values:
        
        newItem = self.unconvertBoard(item)
        a,b,c = cmb.compareRots(yy,newItem)
        maxCompare = max(b)
        if maxCompare > maxPrev:
            maxSim = a
            
        maxPrev = maxCompare
        print('Max compare',maxCompare,maxPrev)
        
        d = self._q_values[item]
        
        print('Qtable',self._q_values)
        print('compared values',b)
        print('qValues',d)
      ...
      print('_________ for loop to add to qTable')
      for i in maxSim:
        pass
